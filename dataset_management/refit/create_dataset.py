...
DATA_DIRECTORY = "D:\\Workspace\\Work\\EVN\\Data\\REFIT"
SAVE_PATH = "D:\\Workspace\\Work\\EVN\\Data\\REFIT"
# DATA_DIRECTORY = os.path.join(cwd, 'data/REFIT')
# SAVE_PATH = os.path.join(cwd, 'data/REFIT')
...
def load(path, building, appliance, channel):
    ...
    # load csv
    file_name = os.path.join(path, 'CLEAN_House' + str(building) + '.csv')
...
def main():
    ...
    for idx, filename in enumerate(os.listdir(path)):
        ...
        if filename == 'CLEAN_House' + str(params_appliance[appliance_name]['test_house']) + '.csv':
            ...
            # Save
            test.to_csv(os.path.join(save_path, appliance_name + '_test_' + 'H' + str(params_appliance[appliance_name]['test_house'])
                        + '.csv'), index=False)
            ...
            # Save
            val.to_csv(os.path.join(save_path, appliance_name + '_validation_' + 'H' + str(params_appliance[appliance_name]['validation_house'])
                       + '.csv'), index=False)
        ...
        elif re.search(r'\d+', filename) is not None and int(re.search(r'\d+', filename).group()) in params_appliance[appliance_name]['houses']:
            print('File: ' + filename)
            ...
            try:
                ...
                if filename == 'CLEAN_House' + str(params_appliance[appliance_name]['test_on_train_house']) + '.csv':
                    csv.to_csv(os.path.join_path(save_path, appliance_name + '_test_on_train_' + 'H' + str(
                        params_appliance[appliance_name]['test_on_train_house'])+ '.csv'), index=False)
                ...
                csv.to_csv(os.path.join(save_path, appliance_name + '_training_.csv'), mode='a', index=False, header=False)
