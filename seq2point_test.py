...
class Tester():
    ...
    def test_model(self):
        ...
        steps_per_test_epoch = np.round(
            int(test_generator.total_size / self.__number_of_windows) - 10, decimals=0)

        # Test the model.
        start_time = time.time()
        testing_history = model.predict(x=test_generator.load_dataset(), steps=steps_per_test_epoch, verbose='auto')
        ...
        self.plot_results(testing_history, test_input, test_target, 10000, 20000)
    ...
    def plot_results(self, testing_history, test_input, test_target, start, end):
        ...
        test_input = (test_input.flatten() * mains_data["std"]) + mains_data["mean"]

        test_input[self.__window_offset: -self.__window_offset] # Remove offset
        test_input = test_input[start: end]
        test_target = test_target[start: end]
        testing_history = testing_history[start:end]
        ...
        plt.figure(1)
        plt.plot(test_input, label="Aggregate")
        ...
        # test_input = test_input[:testing_history.size]

        # Plot testing outcomes against ground truth.
        # plt.figure(1)
        # plt.plot(test_input[self.__window_offset: -self.__window_offset], label="Aggregate")
        # plt.plot(test_target[:test_input.size - (2 * self.__window_offset)], label="Ground Truth")
        # plt.plot(testing_history[:test_input.size - (2 * self.__window_offset)], label="Predicted")
        # plt.title(self.__appliance + " " + self.__network_type + "(" + self.__algorithm + ")")
        # plt.ylabel("Power Value (Watts)")
        # plt.xlabel("Testing Window")
        # plt.legend()
